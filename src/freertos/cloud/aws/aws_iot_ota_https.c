...
#define LIBRARY_LOG_NAME    "OTA-HTTPS"
                         ...
                         char *pPath, size_t pathLen)
{
    ...
    {
        LogError( ("URL must start with https:// (got: %.20s...)\r\n", pUrl) );
        return -1;
    }
    ...
    LogInfo( ("URL parsed: host=%s, path=%.50s\r\n", pHost, pPath) );
    return 0;
}
...
int ota_https_download(const ota_https_req_t *pReq, uint32_t *pBytesReceived)
{
    ...
    LogInfo( ("Starting download from: %.50s...\r\n", pReq->pURL) );
    ...
    {
        LogError( ("TLS connection failed (status=%d)\r\n", tlsStatus) );
        return -1;
    }

    LogInfo( ("TLS connection established\r\n") );
    ...
    LogInfo( ("Before InitializeRequestHeaders:\r\n") );
    LogInfo( ("  host='%s' (len=%u)\r\n", host, (unsigned)strlen(host)) );
    LogInfo( ("  path='%s' (len=%u)\r\n", path, (unsigned)strlen(path)) );
    LogInfo( ("  buffer=%p, bufferLen=%u\r\n", requestHeaders.pBuffer, (unsigned)requestHeaders.bufferLen) );

    /* Reinitialize with method and path info */
    httpStatus = HTTPClient_InitializeRequestHeaders(&requestHeaders, &requestInfo);
    if (httpStatus != HTTPSuccess)
    {
        LogError( ("HTTPClient_InitializeRequestHeaders failed: %d\r\n", httpStatus) );
        goto cleanup;
    }

    LogInfo( ("HTTP GET %s%s\r\n", host, path) );
    ...
    if (httpStatus != HTTPSuccess)
    {
        LogError( ("HTTPClient_Send failed: %d\r\n", httpStatus) );
        goto cleanup;
    }

    LogInfo( ("HTTP response status: %u\r\n", response.statusCode) );

    /* Verify HTTP 200 OK */
    if (response.statusCode != 200u)
    {
        LogInfo( ("Unexpected HTTP status: %u\r\n", response.statusCode) );
        goto cleanup;
    }

    /* Get Content-Length for progress tracking */
    size_t contentLength = response.contentLength;
    LogInfo( ("Content-Length: %lu bytes\r\n", (unsigned long)contentLength) );
    ...
    LogInfo( ("Streaming firmware body...\r\n") );
    ...
    if (response.bodyLen > 0)
    {
        ...
        {
            LogError( ("dataCb returned error at offset 0\r\n") );
            goto cleanup;
        }
        totalBytesReceived += response.bodyLen;
        LogInfo( ("Downloaded %lu bytes\r\n", (unsigned long)totalBytesReceived) );
    }
    ...
    while (totalBytesReceived < contentLength)
    {
        ...
        if (bytesRead == 0)
        {
            ...
            LogInfo( ("EOF before Content-Length (got %lu, expected %lu)\r\n",
        ...
        {
            LogError( ("dataCb returned error at offset %lu\r\n",
                      (unsigned long)totalBytesReceived) );
            goto cleanup;
        }
        ...
        {
            LogInfo( ("Downloaded %lu / %lu bytes\r\n",
                      (unsigned long)totalBytesReceived, (unsigned long)contentLength) );
        }
    }

    LogInfo( ("Download complete: %lu bytes\r\n", (unsigned long)totalBytesReceived) );
